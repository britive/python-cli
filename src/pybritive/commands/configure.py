...
def tenant(ctx, configure_tenant, configure_alias, output_format, configure_prompt):
    ...
    if not no_prompt:
        if not tenant_name:
            tenant_name = click.prompt('The name of the tenant: [tenant].britive-app.com', type=str)
        if not alias:
            alias = click.prompt(
        ...
        if not output_format:
            output_format = click.prompt(
                'Output format (csv|json|table|yaml): ', default='json', type=output_format_choices
            )
    ...
    ctx.obj.britive.configure_tenant(tenant=tenant_name, alias=alias, output_format=output_format)
...
def global_command(ctx, configure_tenant, output_format, configure_prompt, configure_backend):
    ...
    if not no_prompt:
        if not default_tenant_name:
            default_tenant_name = click.prompt('The default tenant name', type=str)
        if not output_format:
            output_format = click.prompt(
                'Output format (csv|json|table|yaml)', default='json', type=output_format_choices
            )
        if not output_format or output_format not in output_format_choices.choices:
            ...
            output_format = 'json'
        if not backend:
            backend = click.prompt('Britive temporary credential storage backend', default='file', type=backend_choices)

    ctx.obj.britive.configure_global(
        default_tenant_name=default_tenant_name, output_format=output_format, backend=backend
    )


@configure.command(name='import')  # have to specify the name since import is a reserved word
@build_britive
@britive_options(names='silent,token,federation_provider')
def import_command(ctx, silent, token, federation_provider):  # silent is handled by @build_britive
